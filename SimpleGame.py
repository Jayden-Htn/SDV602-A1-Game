...
def make_a_window():
    ...
    buttons = [PSG.Button(
        'Enter',  bind_return_key=True), PSG.Button('Exit')]
    ...
    layout = [[PSG.Image(r'images/forest.png', size=(300, 300), key="-IMG-"), 
               PSG.Text(CMD.show_current_place(), size=(39, 12), font='Any 12', key='-OUTPUT-')],
               [command_col]] # size is the width and height of the story text box
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if event == 'Enter':
            ...
            # update the image
            window['-IMG-'].update(r'images/'+CMD.game_places[CMD.game_state]
                                   ['Image'], size=(300, 300))
            # update background with PSG reskinner
            new_theme = CMD.game_places[CMD.game_state]['Theme']
            RSKN.animated_reskin(window=window,
                            new_theme=new_theme,
