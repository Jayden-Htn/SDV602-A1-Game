"""This is the main module for the game and handles the game window.
...
def make_a_window():
    ...
    prompt_input = [
        ...
        PSG.Input(key="-IN-", size=(20, 1), 
                  font="Any 14", do_not_clear=False),
    ]
    buttons = [PSG.Button("Enter", bind_return_key=True), PSG.Button("Exit")]
    command_col = PSG.Column([prompt_input, buttons], 
                             element_justification="r")
    layout = [
        [
            PSG.Image(r"images/forest.png", size=(300, 300), key="-IMG-"),
            PSG.Text(
                CMD.show_current_place(), 
                size=(39, 12), 
                font="Any 12", 
                key="-OUTPUT-"
            ),
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if event == "Enter":
            ...
            window["-IMG-"].update(
                r"images/" 
                + CMD.game_places[CMD.game_state]["Image"], 
                size=(300, 300)
            )
