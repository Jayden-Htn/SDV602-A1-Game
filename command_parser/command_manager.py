...
def move(game_place, extra=''):
    """
    Moves the player to a new location and returns the story at that location.
...
def prepFight(event):
    """
    Adds correct actions to the boss fight state, updates the story actions and moves to the boss state.
...
def display_combat_actions():
    """
    Updates the story actions for the boss fight state to match the inventory.
    """
    global game_places
...
def game_play(command_input):
    ...
    Returns:
        story_result (str): calls token validation and returns the story at the new location or other messages
    """
    global game_state
    story_result = ''
